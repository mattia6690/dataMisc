...
dwd.phen<- dwd.phen.urls %>% 
  select(reporter,Type1,Type2,Timeframe,L2_URL) %>% 
  mutate(reporter=str_replace(reporter,"_reporters","")) %>% 
...
# Write the DWDphen table
saveRDS(dwd.phen,"DWDdata/Phenology/DWDphentable.rds")
